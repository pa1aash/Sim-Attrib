...
| **O-16** | **Measure `p_sel`** — the probability a null draw lands in the observed selection cell | agent | **DECIDES WHETHER THE NEXT SESSION'S WORK IS AFFORDABLE. Deferred a third time (G3 specified it, G4 was scoped away from it, G5 halted before it).** Cheap: needs only null draws and the selection rule, no MMC. The cost gate is pre-registered in `audit/MMC_COMPOSITION_SPEC.md` §4. **Note: it is blocked by nothing.** Q-13 and Q-14 both state explicitly that they do not block it; G5's halt was its stop rule, not a finding. **One thing must be specified before it can be measured at all:** `MMC_COMPOSITION_SPEC.md` §6 leaves `T_k` *"not specified"* and *"deferred"*, and `p_sel` is a property of the cell `T_k` defines. |
| **O-17** | **Run an adversarial critic** — against the **MMC composition** | agent | **STILL UNPAID ON ITS OLDER HALF, sixth session.** G5 attacked the diagnostic again, not the composition, which remains both unimplemented and unrefuted. **PARTLY DISCHARGED 2026-08-20 (G4).** A critic pass ran against G3's Phase 2 *numbers* — `audit/G3_ADVERSARIAL_REVIEW.md` — and changed the answer, as it did both previous times the debt was paid by accident. **The composition itself is still unrefuted by anyone**, which is what `audit/MMC_COMPOSITION_SPEC.md` §6 says and what this row now tracks. Note also that G4's pass was **not independent**: the same project, its own code, the same session. |
...
| **O-23** | **Answer Q-14** — one distortion parameter per component, or two | OPERATOR | **BLOCKING for the paper's separability sentence**, jointly with Q-13 which it does not supersede. `S_B` separates under all eight declarable family assignments and fails when a component carries two distortion parameters at once. `audit/K6_SPECTRUM_CHECK.md`. **P-3.** |
| **O-24** | **Choose a path — `docs/DECISIONS.md` D-12** | OPERATOR | **PROPOSED, not decided.** Path 1: measure the cost gate, then build the composition. Path 2: the diagnostic-only pivot of `audit/PIVOT.md`. The evidence favours Path 1; the argument against it is that six sessions say attack before you build. Also the place to take the other reading of `DEVIATIONS.md` D-13. **P-2.** |
| **O-25** | **Sign or reject gate G5** | OPERATOR | `GATES.md`. `ready for review — UNSIGNED`. Report: `audit/S5_REPORT.md`. **P-1.** |
